...
> **Provenance & licensing note (added in Phase 3 review).** The **shipped**
> mapping data (`data/`) is authored from two clean sources only: (1) the
> maintainer's own prior **MIT-licensed** Mukti map (© Aninda Sundar Howlader =
> GRU-953, retained with MIT attribution), and (2) the cited Unicode / Microsoft
> OpenType specifications below. The third-party `rabiulislam-xyz` Python
> converter has **no licence file** (treat as all-rights-reserved); it was used
> ONLY as a *disposable cross-check oracle* to validate that our test
> *expectations* are correct (those expectations are linguistic facts — the
> correct Bengali for a word — not its code). **No code is copied from it into
> the repo.** The factual key→glyph rows below are determined by the SutonnyMJ
> font's own byte layout, cross-checked against multiple converters + the Unicode
> chart.

Research date: 2026-06-22. Mappings below are cross-checked against the cited
sources and confirmed by running an independent open-source converter as a
disposable oracle. Anything not verified is flagged in "CONFIDENCE & CAVEATS".
...
hosonto between two consonants. Each ligature glyph maps to its Unicode C+্+C
sequence. These are factual glyph→Unicode correspondences fixed by the font's own
byte layout (cross-checked against several converters); representative slots:

| Glyph | Unicode | | Glyph | Unicode |
|---|---|-|---|---|
| `¨` | ্য (ya-phala) | | `Ö` `ª` `«` | ্র (ra-phala) |
| `©` | র্ (reph) | | `³` | ক্ত |
| `¶` `ÿ` | ক্ষ | | `¹` | জ্ঞ |
| `½` | ঙ্গ | | `Î` | ত্র |
| `×` | দ্ধ | | `^` | ্ব |
